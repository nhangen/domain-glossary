...
while IFS=$'\t' read -r name glossary repos_csv; do
  ...
  # Run drift-check, parse counts. Always JSON internally for parsing.
  RAW_OUTPUT="$("$DRIFT_CHECK" "${REPO_ARGS[@]}" --json "$glossary" 2>/dev/null || true)"
...
if [[ "$JSON" == 1 ]]; then
  printf '{"summary":true,"resolved":%d,"relocated":%d,"unresolved":%d,"glossaries_missing":%d}\n' \
    "$TOTAL_RESOLVED" "$TOTAL_RELOCATED" "$TOTAL_UNRESOLVED" "$TOTAL_MISSING"
else
  echo
  echo "SUMMARY  RESOLVED=$TOTAL_RESOLVED  RELOCATED=$TOTAL_RELOCATED  UNRESOLVED=$TOTAL_UNRESOLVED  MISSING_GLOSSARIES=$TOTAL_MISSING"
fi

DRIFT=$((TOTAL_RELOCATED + TOTAL_UNRESOLVED + TOTAL_MISSING))
